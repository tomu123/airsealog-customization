report 58001 "Report Customer Detailed Aging"
{
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem(Header; "Integer")
        {
            ...
            column(RoutingNoCaptionLbl; RoutingNoCaptionLbl)
            {
            }
            column(SalesShipmentNoCaptionLbl; SalesShipmentNoCaptionLbl)
            {
            }
            column(PurchaseOrderNoCaptionLbl; PurchaseOrderNoCaptionLbl)
            {
            }
            dataitem(Customer; Customer)
            {
                ...
                dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
                {
                    ...
                    column(RoutingNo; RoutingNo)
                    {
                        AutoFormatType = 1;
                    }
                    column(SalesShipmentNo; SalesShipmentNo)
                    {
                        AutoFormatType = 1;
                    }
                    ...
                    begin
                        ...
                        fnSearchLinesDocument("Document No.");
    ...
    var
        ...
        RoutingNoCaptionLbl: Label 'N° Routing';
        RoutingNo: Code[20];
        SalesShipmentNoCaptionLbl: Label 'No. Guía Remisión';
        SalesShipmentNo: Code[20];
        PurchaseOrderNoCaptionLbl: Label 'Orden Compra';
    ...
    local procedure fnSearchLinesDocument(var DocumNo: Code[20])
    var
        lRecSalesInvoice: Record "Sales Invoice Header";
    begin
        lRecSalesInvoice.Reset();
        lRecSalesInvoice.SetRange("No.", DocumNo);
        if lRecSalesInvoice.FindFirst() then begin
            RoutingNo := lRecSalesInvoice."Routing No.";
            SalesShipmentNo := lRecSalesInvoice."No. Sales Shipment";
            PurchaseOrderNo := lRecSalesInvoice."Purchase order No.";
        end;
    end;
}